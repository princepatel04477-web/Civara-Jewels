...
Catalog.products.forEach((p) => {
  if (!p.id || !p.title || !p.priceINR || p.priceINR <= 0) {
    console.error(`❌ Product missing required fields: ${JSON.stringify(p)}`);
    errors++;
  }
  if (!p.images || p.images.length === 0 || !p.images[0]) {
    console.error(`❌ Product ${p.id} has no valid main image`);
    errors++;
  }
  if (!p.hallmark || !p.hallmark.includes("750")) {
    console.warn(`⚠️ Product ${p.id} hallmark warning: expected BIS 750, got ${p.hallmark}`);
  }
});
